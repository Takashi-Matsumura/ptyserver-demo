...
現在のステータス: **Step 6 完了（ネットワーク遮断モード）**。UI のボタンで `NetworkMode: bridge`（通常）⇔ `NetworkMode: none`（外部通信遮断）を切替可能。遮断モードでは `lo` だけが残り、DNS も名前解決不可、`curl https://...` や `apt update` は失敗する。ベースは Step 5 の自前イメージ。
...
Next.js 16 custom server (server.ts)
  ...
  Step 6: NetworkMode: bridge / none をUIから切替                 ← いまここ
...
- [ ] **Step 7 (未定)**: 複数セッション UI、イメージ選択、など
