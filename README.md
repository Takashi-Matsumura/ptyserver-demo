...
現在のステータス: **Step 4 完了（永続コンテナ + 明示破棄 API）**。単一の Docker コンテナを明示破棄までずっと維持し、ブラウザ切断・PC スリープ・タブの閉じ開きを跨いでも同じ環境に戻れる。
...
Next.js 16 custom server (server.ts)
  ...
  Step 4: 単一の永続コンテナ (ptyserver-demo-shell) を reuse     ← いまここ
```
...
サーバ起動時に `ubuntu:24.04` イメージが無ければ 1 度だけ pull する。ブラウザで `http://localhost:3000` を開くと xterm.js のターミナルが表示され、Ubuntu コンテナ内の bash プロンプト `root@<container-id>:~#` が出る。
...
lib/
  ...
  docker-session.ts        # dockerode ↔ WebSocket 中継 / 永続コンテナの ensure・remove
components/
...
| `apt install` したパッケージ（`/usr`, `/var/lib` 配下） | ✅ 残る | ❌ 消える |
...
- [ ] **Step 5 (未定)**: 自前イメージ (`Dockerfile` 化で vim/git/tool 同梱)、複数セッション UI、イメージ選択、ネットワーク遮断モード、など
