...
現在のステータス: **Step 7 完了（UI 磨き込み）**。ターミナル領域の半透明化、Ubuntu ロゴのウォーターマーク、ヘッダのトグルスイッチ化など見た目を整備。Step 6 までで作り込んだネットワーク遮断モード／永続コンテナ／自前イメージはそのまま機能する。

![Ubuntu ロゴをウォーターマークにしたターミナル画面と、ヘッダのネットワークトグル／コンテナ再作成アイコン](./docs/screenshot.jpeg)
...
Next.js 16 custom server (server.ts)
  ...
  Step 5: docker/sandbox/Dockerfile から自前イメージ
          (ptyserver-demo-sandbox:latest) を build & 再利用
  Step 6: NetworkMode: bridge / none をUIから切替
          （遮断モード: lo のみ、DNS・外部通信不可）
  Step 7: ターミナル領域を半透明化 + Ubuntu ロゴ          ← いまここ
          + ヘッダのトグルスイッチ化 + 再作成アイコン化
```
...
- **Step 6**: UI のボタンからコンテナの `NetworkMode` を **bridge（通常）⇔ none（外部通信遮断）** で切り替えできるようにした。Docker の `NetworkMode` は create 時に確定するため、切替時は `setSessionNetworkMode()` が既存コンテナを `force remove` し、次の WebSocket 接続で新モードで作り直す。現状は `POST /api/session { networkMode }` で切替、`GET /api/session` で現状取得。UI のヘッダに現モードを色付きで表示
- **Step 7**: UI の磨き込み。ターミナル領域の背景を CSS で透明化（`.xterm-viewport` に直書きされた `background: #000` を override）し、裏に indigo 系 gradient を敷く。中央には Ubuntu Circle of Friends ロゴを薄くウォーターマーク表示して、コンテナ内の OS が視覚化されるようにした。ヘッダ側は「ネットワーク接続/遮断」をトグルスイッチ、「コンテナ再作成（旧セッション破棄）」を回転矢印アイコンに置き換えて、強い言葉を避け誤操作しづらいデザインに変更。右端のスクロールバーも非表示にしてクリーンな見た目にした
...
| method | path | body | 用途 |
|--------|------|------|------|
| `GET` | `/api/session` | — | 現在のセッションコンテナの状態を返す `{ exists, running, networkMode }` |
| `POST` | `/api/session` | `{ networkMode: "bridge" \| "none" }` | ネットワークモード切替。変更があれば force remove → 次の WebSocket 接続で新モードで作成 |
...
lib/
  ...
  docker-session.ts        # dockerode ↔ WebSocket 中継 / 自前イメージ build / 永続コンテナの ensure・remove / NetworkMode 切替
components/
  Terminal.tsx             # xterm.js + FitAddon の client component。再接続・ネットワークトグル・コンテナ再作成
app/
  globals.css              # Tailwind + xterm 要素の透過 override、スクロールバー非表示など (Step 7)
  layout.tsx / page.tsx    # dynamic import (ssr:false) で Terminal をマウント
  api/session/route.ts     # GET/POST/DELETE /api/session（状態取得・ネットワーク切替・破棄）
public/
  ubuntu-logomark.svg      # ターミナル背景に薄く重ねる Ubuntu ロゴ (Step 7)
docs/
  screenshot.jpeg          # README に貼り付けている UI スクリーンショット
```
...
UI のヘッダで「ネットワークを遮断」を押すと、`NetworkMode: none` のコンテナに作り直される。この状態では:

| 操作 | bridge（通常） | none（遮断） |
|------|----------------|--------------|
| `curl https://example.com` | ✅ 200 応答 | ❌ DNS/接続失敗 |
| `apt update` / `apt install` | ✅ 動く | ❌ ミラー到達不可 |
| `ping 8.8.8.8` | ✅（capability 次第、本設定では到達） | ❌ `Network is unreachable` |
| `git clone https://...` | ✅ | ❌ |
| `ip addr`（`procps` 内蔵なし、busybox `ip` も無し、`/proc/net/*` で代用） | `lo` + `eth0` | `lo` のみ |
| `/root` 配下の読み書き、vim 編集、bash スクリプト実行 | ✅ | ✅（ローカル作業は完全に可能） |

「遮断モードでも Step 5 の自前イメージに焼き込んだツール（vim/git/curl のバイナリ自体）は使える」のがポイント。`git` は使えるがリモート操作は失敗する、というレベルで外だけが遮断される。本当にネットワークから切り離されたサンドボックスが必要な時に切り替える想定。
...
### xterm.js の半透明化は `theme.background` の rgba では効かない

xterm v6 の `ThemeService.parseColor` は `canvas.fillStyle` で validate する際に `alpha !== 0xFF` を reject するため、`theme.background` に `rgba(...)` を渡しても **黒へサイレントフォールバック** する。加えて同梱の `xterm.css` は `.xterm .xterm-viewport` に `background-color: #000` を直書きしている。このため半透明 UI は、`app/globals.css` 側で `.xterm`, `.xterm *`, `.xterm-viewport`, `.xterm-screen` などの background を `transparent !important` で上書きし、裏に置いた gradient を透かして見せる方式で実装している（cursor と selection の色だけは override 後に個別復活）。
...
- [x] **Step 6**: ネットワーク遮断モード
  - UI のヘッダから `NetworkMode: bridge` ⇔ `none` を切替
  - `NetworkMode` は create 時に確定するため、切替時は既存コンテナを `force remove` して作り直す
  - `GET /api/session` で現状取得、`POST /api/session { networkMode }` で切替
  - 遮断中: `lo` だけの状態。DNS/外部通信不可、ローカル作業（vim/git/bash）は普通に可
- [x] **Step 7**: UI 磨き込み
  - ターミナル領域を半透明化。裏に indigo 系 gradient を敷き、その中央に Ubuntu Circle of Friends ロゴを薄くウォーターマーク表示
  - xterm.js は `theme.background` に rgba を受け付けないため、CSS で `.xterm` 配下の背景を強制 transparent にする方式
  - ヘッダのネットワーク切替をトグルスイッチ化、セッション破棄を回転矢印アイコン化（tooltip で意図を補足）。スクロールバーも非表示
  - `public/ubuntu-logomark.svg` の path データは Simple Icons (CC0)、ロゴ自体は Canonical の商標。サードパーティアセット節を参照
- [ ] **Step 8 (未定)**: 複数セッション UI、イメージ選択、など

## セキュリティ注記

これは **学習用** プロジェクト。Step 2 のコンテナ隔離も基本的なリミットにとどまる。Step 4 でコンテナが永続化されたことで、コンテナ内に残したプロセスやファイルも長期間残るようになっており、信頼できない入力を流す用途にはそもそも向かない。Step 6 で `NetworkMode: none` は入ったが、これは「コンテナ内から外に出られなくする」だけで、コンテナからホストへの脱出対策とは別軸。多ユーザ・本番で使うにはさらに rootless Docker / userns-remap / gVisor などの検討が必要。
...
### Ubuntu ロゴ (`public/ubuntu-logomark.svg`)

ターミナル背景の中央に薄く表示している「Ubuntu Circle of Friends」ロゴについて:

- **SVG path データ**: [Simple Icons](https://github.com/simple-icons/simple-icons) 由来。Simple Icons のアイコンファイルは [**CC0-1.0**（パブリックドメイン相当）](https://github.com/simple-icons/simple-icons/blob/develop/LICENSE.md) で提供されている
- **"Ubuntu" の名称およびロゴ（Circle of Friends）**: [**Canonical Ltd. の商標**](https://ubuntu.com/legal/intellectual-property-policy)。本プロジェクトは Canonical と一切の提携・後援関係を持たない
- **本プロジェクトでの使用目的**: サンドボックスコンテナ内で実行されている OS が Ubuntu であることを視覚的に示すための**装飾（ウォーターマーク）**。商用利用や Canonical との関係を暗示する用途では使用しない
- **色の変更について**: 公式ブランドカラーは **Ubuntu Orange (`#E95420`)** だが、本プロジェクトでは画面全体の indigo 系 gradient に馴染ませるため **indigo (`#6366f1`)** で表示し、さらに opacity 15% の低不透明度で背景扱いにしている。商用の製品表示ではこの改変は認められない点に注意

Ubuntu のロゴと名前の使用に制約のあるフォークや再配布を行う場合は、[Canonical の商標ポリシー](https://ubuntu.com/legal/intellectual-property-policy) を確認してください。

## ライセンス

[MIT License](./LICENSE). 本リポジトリの独自コード（`lib/`, `app/`, `components/`, `docker/`, `server.ts` 等）に適用されます。サードパーティアセットの扱いは上記セクションを参照。
